...
void display(short int gr[size][size], int cluster1[size][size], int cluster0[size][size], int i1, int j1){
	int i,j,k;
	for(i=2;i<size-2;i++){
		for(j=2;j<size-2;j++){
			if(gr[i][j]==1)
...
void f_gen(short int gr[size][size]){
	...
	for(i=0;i<size;i++){
		for(j=0;j<size;j++){
			...
			if(mrg32k3a_generate_uniform_float_(&state_1)<freq)
				gr[i][j]=1;
			else
				gr[i][j]=0;
			}
...
int edge(short int grid[size][size], int cluster1[size][size], int cluster0[size][size], int i1, int j1, int len, int elem)
{
	...
	//cluster1[i1][j1] = 1;
	printf("\ni=%d j=%d\n", i1, j1);
	display(grid, cluster1, cluster0, i1, j1);
	getchar();	
	if(i1 == size-3)
		return len;
	...
	if((cluster1[i1+1][j1] == elem) && (flag == 0)){
                len++;
                flag=edge(grid, cluster1, cluster0, i1+1, j1, len, elem);
        }

	if((cluster1[i1+1][j] == elem) && (flag == 0)){
                len++;
                flag=edge(grid, cluster1, cluster0, i1+1, j, len, elem);
        }

	if((cluster1[i1][j] == elem)  && (flag == 0)){
		len++;
		flag=edge(grid, cluster1, cluster0, i1, j, len, elem);
	}
...
int perimeter(short int grid[size][size], int cluster1[size][size], int cluster0[size][size], int root1, int root2, int* def, int* cop)
{
	...
	for(j=2;j<size-2;j++)
	{
		if(cluster1[2][j]==-1)
		{
			...
			jstart=j;
			//printf("%d %d\n", i1, j1);
			cluster_edge=edge(grid,cluster1, cluster0, i1, j1, cluster_edge, -1);
			printf("no edge %d", cluster_edge);
			if(cluster_edge != 0)
				break;
		}
		if(j==size-3)
...
int percolate1(short int grid[size][size], int cluster[size][size],int cluster0[size][size])
{	
	perc_size=0;
...
int percolate(short int grid[size][size], int cluster1[size][size], int cluster0[size][size])
{
        perc_size=0;
        int i, j, k, flag=0, l, root1, root2, per=0;
        ...
        for(j=2;j<size-2;j++)
        {
                ...
                if(cluster1[2][j]!=0)
                {
                        root1=find_root(cluster1[2][j], LL);
                        for(i=0;i<perc_size;i++)
                        {
                                if(root1==perc_arr[i])
                                flag=1;
				 ...
				 for(k=2;k<size-2;k++)
                                        {
                                                if(cluster1[2][k]!=0)
						{
                                                	root2=find_root(cluster1[2][k], LL);
							per=edge(grid,cluster1, cluster0, 2,k,0,1);
						}
                                        }
				if(per>0)
					return root1;
...
double int_average(double* mass, long int n, short int flag)
{
	long int i;
	long int sum=0, up=0, down=0;
	double av=0, var=0;
...
{	
	...
	//printf("coop perc\n");
	coop=percolate(grid, cluster1, cluster0);
	...
	def=percolate(grid, cluster0, cluster1);
...
int main(int argc, char *argv[]){
	short int grid[size][size], i,j, k=0, c=0;
	int c_clusters[100000], d_clusters[15000];
	long int m=0,l=0;
	double fc[N], freq[2000], av_freq[50], c_av[100], c_av_t[1000];
	char str[15], t_str[10], path[10], val[10];
	mt19937_init_sequence_(&state, time(NULL));
	mrg32k3a_init_sequence_(&state_1, time(NULL));
	...
	{	
		...
		while(j<1000)
		{
			c_av_t[j]=c_clusters[j+i*1000];
			j++;
		}
		c_av[i]=int_average(c_av_t, 1000, 0);
	}
	int_average(c_av, 100, 1);
	free(LL);
	free(perc_arr);
	return 0;
}
