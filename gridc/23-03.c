...
void display(short int gr[size][size], short int cluster[size][size]){
	int i,j,k;
	for(i=2;i<size-2;i++){
		for(j=2;j<size-2;j++){
			if(gr[i][j]==1)
...
int main(int argc, char *argv[]){
	short int grid[size][size], i,j, k=0, m=0;
	...
	//{
		...
		int_average(c_clusters, 15000,1);
		int_average(d_clusters, 15000,1);
		T+=0.000001;
		m=0;
	//}	
