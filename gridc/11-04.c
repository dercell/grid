...
{	
	int i,j,k, res=0, c, interval=0;
	int cluster0[size][size]; 
	
	int step=(size-4)*(size-4)/10;
	...
	{	
		...
		{	
			...
			if(param) c_density[interval-1]++;
			else d_density[interval-1]++;
...
int main(int argc, char *argv[]){
	...
	{	
		...
		for(c=0;c<10;c++)
		{
			sprintf(val, "%f", c_density[c]*1.0/15000);
			fputs(val, fin);
			fputs(" ", fin);
		}
		fputs("\n", fin);
		...
		for(c=0;c<10;c++)
		{
			sprintf(val, "%f", d_density[c]*1.0/15000);
			fputs(val,fin);
			fputs(" ", fin);
		}
		fputs("\n", fin);	
